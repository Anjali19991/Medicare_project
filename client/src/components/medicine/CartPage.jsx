            ...
            <div className={`flex justify-between max-[1100px]:flex-wrap mt-4 ${cartItems.length === 0 ? 'min-h-[75vh] items-center' : '' }`}>
                <div className="w-full flex flex-col items-center justify-center px-4">
                                    ...
                                    <tr key={index} className="border-b text-center">
                                        ...
                                        <td className="py-2">
                                            <div className="flex gap-2 items-center justify-center">
                                                ...
                                                <button className="text-2xl text-white px-2 rounded-sm bg-green-600" onClick={() => handleIncreaseQuantity(item)}>+</button>
                                            </div>
                                        </td>
                    ...
                    <div className="flex items-center gap-4">
                        <Link to={"/buymedicines"} className="px-4 py-2 text-white rounded bg-teal-500">Continue Shopping</Link>
                        {
                            cartItems.length > 0 ? (
                                <button
                                    onClick={handleClearCart}
                                    className="bg-red-500 text-white my-8 py-2 px-4 rounded hover:bg-teal-600 transition-all duration-300"
                                >
                                    Clear Cart
                                </button>
                            ) : ("")
                ...
                </div>
                {
                    cartItems.length !== 0 ? (
                        <div className="max-[1100px]:w-full w-1/2 text-center">
                            ...
                            <button className="px-4 py-2 bg-teal-800 text-white mx-auto">Proceed To Buy</button>
                        </div>
                    ) : (
                        ""
                    )
